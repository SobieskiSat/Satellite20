#define Serial_Monitor_Baud 12000000
...
void loop()
{
  ...
  //{
    ...
    while (!stop)
    {
      uint8_t buffersize = packetLength;
      uint8_t buff[packetLength];
      TXPacketL = buffersize;

      buff[0] = (uint8_t)(fragment >> 8);   //SerialUSB.write(buff[0]);
      buff[1] = (uint8_t)(fragment & 0xFF); //SerialUSB.write(buff[1]);
      for (int i = 0; i < packetLength - 2; i++)
      {
        buff[i + 2] = bmp[fragment * (packetLength - 2) + i];
        //SerialUSB.write(buff[i + 2]);
      }
      ...
      if (SX1280LT.sendPacketFLRC(buff, buffersize, 1000, PowerTX, DIO1))
      {
        delay(8);
        fragment++;
        if ((int)(fragment) == (int)((sizeof(bmp)/sizeof(*bmp)) / (packetLength - 2)))
        {
          digitalWrite(LED_BUILTIN, LOW);
          stop = true;
          while (true) {}
